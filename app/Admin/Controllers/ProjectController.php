    ...
    protected function grid()
    {
        return Admin::grid(Project::class, function (Grid $grid) {
            ...
            $grid->column('业主')->display(function(){
            	// return $this->uid > 100000000 ? DB::table('customer')->where('phone',$this->uid)->value('phone').'<br>'.DB::table('customer')->where('phone',$this->uid)->value('name'):DB::table('user')->where('id',$this->uid)->value('phone').'<br>'.DB::table('user')->where('id',$this->uid)->value('name');
                if ($this->uid > 100000000) {
                    // $ye = DB::table('customer')->where('phone',$uid)->select('phone','name')->first();
                    return DB::table('customer')->where('phone',$this->uid)->value('phone').'<br>'.DB::table('customer')->where('phone',$this->uid)->value('name');
                }else{
                    // $ye = DB::table('user')->where('id',$uid)->select('phone','name')->first();
    ...
    protected function form($id = 0)
    {
        return Admin::form(Project::class, function (Form $form) use($id){
            ...
            $form->tab('基本信息', function ($form) use($cid,$id,$role){
                ...
                $form->image('image','首图')->setWidth(3)->help('建议图片比例2:1');
                ...
                $form->hidden('updated_at', '更新时间');
                 ...
                 $form->multipleImage('zxht','装修合同')->removable()->move('zxht');
                 $form->multipleImage('ysqd','预算清单')->removable()->move('ysqd');
                 $form->multipleImage('xmcl','项目材料')->removable()->move('xmcl');
                 $form->multipleImage('xgt','效果图')->removable()->move('xgt');
                 $form->multipleImage('yszp','验收照片')->removable()->move('yszp');
                 $form->multipleImage('fkxx','收付款信息')->removable()->move('fkxx');
            });
            ...
            $form->saving(function (Form $form) {
                ...
                //-----------
                // $yeuid = DB::table('user')->where('phone',$form->uid)->value('id');
                // if ($yeuid) {
                //     DB::table('project')->where('id',$pro_id)->update(['uid'=>$yeuid]);
                // }
