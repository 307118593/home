...
class StaffController extends Controller
{
    ...
    protected function grid()
    {
        return Admin::grid(Staff::class, function (Grid $grid) {
            ...
            $job = admin::user()->job;
            $grid->model()->where('pid','>',0)->orderBy('sort','desc')->orderBy('id','desc');
            if ($pid == 0) {
    ...
    protected function form($id=0)
    {
        return Admin::form(Staff::class, function (Form $form) use($id){
            ...
            $form->select('job','岗位')->options($jobs)->default(2)->setwidth(2)->help('如果岗位是设计师,请完善以下信息,将展示在APP内');
            // if ($id > 0) {
            ...
            // }
             // if ($id > 0) {
                ...
                $form->slider('year','工龄/经验')->options(['max' => 15, 'min' => 2, 'step' => 1, 'postfix' => '年'])->setwidth(4);
                $form->textarea('honor','个人荣誉')->setwidth(5);
                $form->text('content','设计理念')->setwidth(5)->help('设计师的以上信息将会展示在APP里.');
                // $form->divide();
            // }
            if ($role != 3) {
                $form->text('sort','排序权重')->setwidth(2)->help('数字越大首页顺序越靠前,只对岗位设计师有效')->default(0);
                // $states = [
                //     'on'  => ['value' => 1, 'text' => '是', 'color' => 'success'],
                //     'off' => ['value' => 0, 'text' => '否', 'color' => 'default'],
                // ];
